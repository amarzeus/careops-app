...
export default function InventoryPage() {
  ...
  const [forecast, setForecast] = useState<
    Record<string, { daysRemaining: number; confidence: string }>
  >({});
    ...
    try {
      ...
      try {
        ...
        if (forecastRes.ok) {
          ...
          fData.forecast.forEach(
            (f: { name: string; daysRemaining: number; confidence: string }) => {
      ...
      reader.onload = async () => {
        ...
        if (data && data.items && data.items.length > 0) {
          ...
          toast({
            ...
            description: "Could not identify inventory items in the image.",
          });
        }
      };
    } catch (error) {
  ...
  const handleCreateOrUpdate = async (data: Record<string, unknown>) => {
    try {
      const payload = {
        ...
        threshold: parseInt(data.threshold as string),
      };
