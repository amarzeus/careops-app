    ...
    const formatted = await Promise.all(
      conversations.map(
        async (c: {
          ...
          unreadCount?: number;
          lastMessageAt: Date;
          messages: { isAutomated: boolean; direction: string; content: string }[];
        }) => {
          // Find the last inbound message for classification
          const lastInboundMessage = c.messages.find(
            (m: { isAutomated: boolean; direction: string; content: string }) =>
              !m.isAutomated && m.direction === "INBOUND"
          );

          let intent = null;
          if (lastInboundMessage?.content && (c.unreadCount || 0) > 0) {
            try {
              const history = c.messages
                .filter(
                  (m: { direction: string; isAutomated: boolean }) =>
                    m.direction === "INBOUND" && !m.isAutomated
                )
                .slice(0, 3)
