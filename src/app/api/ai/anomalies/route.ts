...
export async function GET() {
    ...
    try {
        const [
            ...
            prisma.booking.count({ where: { workspaceId, createdAt: { gte: lastWeekStart, lt: thisWeekStart } } }),
            ...
            prisma.contact.count({ where: { workspaceId, createdAt: { gte: lastWeekStart, lt: thisWeekStart } } }),
            ...
            prisma.inventoryItem.count({ where: { workspaceId, quantity: { lte: prisma.inventoryItem.fields.threshold as unknown as number } } }),
            prisma.message.count({ where: { conversation: { workspaceId }, direction: "INBOUND", status: { not: "READ" } } }),
        ]);
