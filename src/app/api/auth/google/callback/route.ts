...
export async function GET(req: Request) {
  ...
  if (errorParam) {
    ...
    return NextResponse.redirect(
      new URL(
        `/login?error=google_auth_failed&message=${encodeURIComponent(`Google error: ${errorParam}`)}`,
        baseUrl
      )
    );
  }
  ...
  if (!code) {
    ...
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:5000";
    return NextResponse.redirect(
      new URL("/login?error=google_auth_failed&message=No authorization code", baseUrl)
    );
  }
  ...
  try {
    ...
    if (!googleUser.emailVerified && !googleUser.verified_email) {
      ...
      const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:5000";
      return NextResponse.redirect(
        new URL("/login?error=google_auth_failed&message=Email not verified", baseUrl)
      );
    }
    ...
    if (!user) {
      ...
      console.log(
        `[Google Auth Callback] New user created: ${user.id} with workspace: ${workspace.id}`
      );
    } else {
    ...
    return NextResponse.redirect(
      new URL(
        `/login?error=google_auth_failed&message=${encodeURIComponent(errorMessage)}`,
        baseUrl
      )
    );
  }
