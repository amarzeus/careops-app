...
export async function POST(req: Request) {
  try {
    ...
    console.log(
      `[ResetPassword] Request: method=${method}, email=${email}, phone=${phone}, normalizedPhone=${normalizedPhone}, otp=${otp}`
    );
    ...
    if (!user) {
      console.log(
        `[ResetPassword] User not found for ${method === "email" ? email : normalizedPhone}`
      );
