...
function categorizeError(
  error: TwilioError | unknown,
  statusCode?: number
): { code: TwilioErrorCode; retryable: boolean } {
  ...
  if (
    (message.includes("invalid") && message.includes("number")) ||
  ...
  if (digits.length === 10) {
    if (digits.startsWith("0")) {
      return {
        valid: false,
        formatted: "",
        error: "10-digit number cannot start with 0 (Did you miss the country code?)",
      };
    }
...
export async function sendSMS(to: string, body: string): Promise<TwilioResult> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(
        `[Twilio:sendSMS] Sent to ${formattedPhone}, SID: ${message.sid}, Status: ${message.status}`
      );
...
export async function sendWhatsApp(to: string, body: string): Promise<TwilioResult> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(
        `[Twilio:sendWhatsApp] Sent to ${formattedPhone}, SID: ${message.sid}, Status: ${message.status}`
      );
      ...
      console.error(
        `[Twilio:sendWhatsApp] Attempt ${attempt} failed: ${err.message} (code: ${code})`
      );
