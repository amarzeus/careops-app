    ...
    const fetchBadges = async () => {
      try {
        ...
        if (alertsRes.ok) {
          ...
          const inboxCount = unreadAlerts.filter(
            (a: { type: string }) => a.type === "message"
          ).length;
          const bookingCount = unreadAlerts.filter(
            (a: { type: string }) => a.type === "booking"
          ).length;
        ...
        <Link
          ...
          className={cn(
            ...
            isActive
              ...
              : "text-muted-foreground hover:text-foreground hover:bg-[var(--sidebar-item-hover-bg)] dark:hover:text-slate-200"
          )}
        >
          <item.icon
            className={cn(
              ...
              isActive
                ...
                : "text-muted-foreground group-hover:text-muted-foreground dark:group-hover:text-slate-300"
            )}
          />
          ...
          {collapsed && (
            <span className="border-border bg-popover text-popover-foreground pointer-events-none absolute left-full z-50 ml-2.5 hidden rounded-lg border px-2.5 py-1.5 text-xs font-medium whitespace-nowrap shadow-lg group-hover:block">
              {item.label}
              {badgeCount > 0 && (
  ...
  return (
    <>
      {/* ─────────────────────────────────────────
          ...
          ───────────────────────────────────────── */}
      <aside className="fixed top-0 left-0 z-40 flex h-screen w-16 flex-col border-r border-[var(--sidebar-border)] bg-[var(--sidebar-bg)] lg:hidden">
        {/* Logo icon */}
        ...
        <button
          className="hover:bg-muted/50 mt-2 flex h-9 w-9 cursor-pointer items-center justify-center self-center rounded-lg transition-colors dark:hover:bg-slate-800"
          onClick={() => setMobileOpen(true)}
          aria-label="Open menu"
        >
          <Menu className="text-muted-foreground h-4.5 w-4.5" />
        </button>

        {/* Collapsed nav */}
        <nav className="mt-2 flex flex-col gap-0.5 px-2">{renderNavItems(true)}</nav>

        {/* User avatar in rail */}
        <div className="mt-auto flex flex-col items-center justify-center gap-2 border-t border-[var(--sidebar-border)] p-2 pt-4">
          <ThemeToggle />
          <button
      ...
      {/* ─────────────────────────────────────────
          ...
          ───────────────────────────────────────── */}
      <aside className="fixed top-0 left-0 z-40 hidden h-screen w-64 flex-col border-r border-[var(--sidebar-border)] bg-[var(--sidebar-bg)] lg:flex">
        {/* Header / Logo */}
        <div className="flex h-14 shrink-0 items-center gap-3 border-b border-[var(--sidebar-border)] px-5">
          ...
          >
            ...
            <div className="flex min-w-0 flex-col justify-center">
              <span className="text-foreground text-lg leading-none font-bold tracking-tight">
                Care<span className="text-primary">Ops</span>
              </span>
              <p className="text-muted-foreground/80 max-w-[140px] truncate text-[10px] font-semibold tracking-wider uppercase">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-3">
          <div className="flex flex-col gap-0.5">{renderNavItems(false)}</div>
        </nav>
        ...
        <div className="shrink-0 border-t border-[var(--sidebar-border)] p-3">
          <div className="mb-2 flex items-center gap-3 rounded-lg px-2 py-1.5">
            <div className="text-primary/90 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-50 text-sm font-semibold dark:bg-blue-900/40 dark:text-blue-300">
              {initials}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-foreground truncate text-sm font-medium">{userName || "User"}</p>
          ...
          <Button
            variant="ghost"
            size="sm"
            className="text-muted-foreground w-full justify-start gap-2 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
            onClick={handleLogout}
          >
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 z-50 h-screen w-72 flex-col border-r border-[var(--sidebar-border)] bg-[var(--sidebar-bg)] shadow-xl transition-transform duration-300 ease-in-out lg:hidden",
      ...
      >
        ...
        <div className="flex h-14 items-center justify-between border-b border-[var(--sidebar-border)] px-5">
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="hover:bg-muted/50 flex h-8 w-8 items-center justify-center rounded-lg transition-colors dark:hover:bg-slate-800"
        ...
        <div className="shrink-0 border-t border-[var(--sidebar-border)] p-3">
          <div className="mb-2 flex items-center gap-3 rounded-lg px-2 py-1.5">
            <div className="text-primary/90 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-50 text-sm font-semibold">
              {initials}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-foreground truncate text-sm font-medium">{userName || "User"}</p>
          ...
          <Button
            variant="ghost"
            size="sm"
            className="text-muted-foreground w-full justify-start gap-2 hover:bg-red-50 hover:text-red-600"
            onClick={handleLogout}
          >
            <LogOut className="h-3.5 w-3.5" />
            <span className="text-xs">Sign Out</span>
